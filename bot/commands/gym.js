...
module.exports = async function gymCommand(interaction, client) {
  ...
  if (sub === 'register') {
  // Acknowledge interaction quickly because we'll do async operations (send message + add reactions)
  try { await interaction.deferReply({ ephemeral: true }); } catch (_) {}
  // Instead of parsing a string, create a message with reactions for user to pick days
    const channel = interaction.channel;
    const text = 'React to select the days you plan to go to the gym:\n\n' +
      '🟥 SU  🟩 M  🟦 T  🟨 W  🟪 TH  🟫 F  ⬜ SA\n\n' +
      'React with the day emojis to choose your schedule, then react ✅ when finished.';
    // Send a visible channel message so reactions are clickable by users
  ...
  if (!msg) return interaction.editReply({ content: 'Unable to create registration message.' });
    // add reactions for day selection using specific emojis
    const mapping = [ ['SU','🟥'], ['M','🟩'], ['T','🟦'], ['W','🟨'], ['TH','🟪'], ['F','🟫'], ['SA','⬜'] ];
    for (const [, emoji] of mapping) {
      try { await msg.react(emoji); } catch (_) { }
    }
    // add a finalize reaction the user will press when they've finished selecting days
    try { await msg.react('✅'); } catch (_) {}
    // Save pending registration tied to this message
